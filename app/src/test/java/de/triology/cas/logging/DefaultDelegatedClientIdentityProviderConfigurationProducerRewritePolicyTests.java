...
/**
 * Unit tests for {@link DefaultDelegatedClientIdentityProviderConfigurationProducerRewritePolicy}.
 */
class DefaultDelegatedClientIdentityProviderConfigurationProducerRewritePolicyTests {

    private DefaultDelegatedClientIdentityProviderConfigurationProducerRewritePolicy policy;

    @BeforeEach
    void setUp() {
        policy = DefaultDelegatedClientIdentityProviderConfigurationProducerRewritePolicy.createPolicy();
    }

    @Test
    void createPolicy_ShouldReturnInstance() {
        // when
        DefaultDelegatedClientIdentityProviderConfigurationProducerRewritePolicy instance =
                DefaultDelegatedClientIdentityProviderConfigurationProducerRewritePolicy.createPolicy();
    ...
    void rewrite_ShouldMaskPassword_WhenPasswordPresent() {
        // given
        String message = "Authentication flow with attributes: 'password' -> 'mySecret', 'exec' -> 'runCommand'";
        ...
        assertFalse(modifiedMessage.contains("mySecret"), "Original password should not appear");
    }

    @Test
    void rewrite_ShouldReturnOriginal_WhenNoPasswordFlagPresent() {
        // given
        String message = "Authentication attempt with no password field";
        LogEvent event = Log4jLogEvent.newBuilder()
                .setMessage(new SimpleMessage(message))
                .build();
